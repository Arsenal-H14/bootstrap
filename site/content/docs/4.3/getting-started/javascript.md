...
All constructors accept an optional options object or nothing (which initiates a plugin with default behavior):
...
If you'd like to get a particular plugin instance, each plugin expose `_getInstance` method to retrieve it directly from an element: `bootstrap.Popover._getInstance(myPopoverEl)`.
